...
import type { Service, Category } from '@/types/pricing';
  ...
  const icons: Record<string, React.ReactNode> = {
    shirt: (
      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
        <path d="M16 21v-2l-8-5V8c0-1.103.897-2 2-2h4c1.103 0 2 .897 2 2v6l-8 5v2h8zm-8-9v3.764l4 2.5V14H8zm8 0h-4v4.264l4-2.5V12zm-4-4H8v2h4V8z"/>
      </svg>
    ),
    washing: (
      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
    ...
    jacket: (
      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2C9.79 2 8 3.79 8 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2h-2V6c0-2.21-1.79-4-4-4zm2 6h-4V6c0-1.1.9-2 2-2s2 .9 2 2v2zm4 12H6V10h12v10z"/>
      </svg>
    ),
    shoe: (
      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12.83 6.6l-1.76 1.76-1.76-1.76L10.83 5l1.76 1.76L14.83 5l1.76 1.76 1.76-1.76 1.42 1.42-1.76 1.76 1.76 1.76-1.42 1.42-1.76-1.76-1.76 1.76L12.83 10l1.76-1.76L12.83 6.6M3 20v2h18v-2c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2z"/>
      </svg>
    ),
    bag: (
      <svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24">
...
export default function PricingPage() {
  const [activeTab, setActiveTab] = useState<string>('dry-clean');
  ...
  const activeCategory: Category | undefined = pricingData.categories.find(
    (cat: Category) => cat.id === activeTab
  );
      ...
      <nav className="sticky top-0 z-50 bg-white shadow-sm px-4 py-4 border-b border-gray-200">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <Link href="/stores" className="text-gray-700 hover:text-teal-600">Our Stores</Link>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden mt-4 pb-4 border-t border-gray-200 bg-white">
            <div className="flex flex-col space-y-2 pt-4 max-w-7xl mx-auto">
              ...
              <Link href="/pricing" className="py-2 px-2 text-teal-600 font-semibold hover:bg-gray-50 rounded">Pricing</Link>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-10">
            <div className="inline-block bg-teal-600 text-white px-5 py-2 rounded-full text-sm font-semibold mb-3">
              Transparent Pricing
            </div>
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">
              Quality service at affordable prices. No hidden charges, transparent billing.
            </p>
          </div>
          {/* Tabs */}
          <div className="flex justify-center gap-3 mb-12">
            {pricingData.categories.map((category: Category) => (
              <button
                key={category.id}
          ...
          {activeCategory && activeCategory.services.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {activeCategory.services.map((service: Service) => (
                <div
                  key={service.id}
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg hover:border-teal-200 transition-all duration-300"
                >
